...
EXPECTED_NODE_HOSTNAME="${EXPECTED_NODE_HOSTNAME:-cp1}"
...
run_phase_with_progress() {
  ...
  local cmd_pid start_ts now elapsed last_line img_count msg rc
  start_ts="$(date +%s)"
  rc=0

  timeout --foreground --signal=TERM --kill-after=60 "${timeout_seconds}" "$@" >>"${KUBEADM_LOG}" 2>&1 &
  cmd_pid="$!"

  while kill -0 "${cmd_pid}" >/dev/null 2>&1; do
    sleep "${CP1_PROGRESS_INTERVAL_SECONDS}" || true
    now="$(date +%s)"
    elapsed="$((now - start_ts))"
    last_line="$(tail -n 1 "${KUBEADM_LOG}" 2>/dev/null || true)"
    img_count="$(count_k8s_images)"
    msg="[cp-init] ${phase}: elapsed=${elapsed}s cached_images=${img_count} last='${last_line}'"
    echo "${msg}"
    echo "${msg}" >>"${CP1_PROGRESS_FILE}"
  done
...
rm -f "${KUBEADM_LOG}"
echo "cp-init script started at $(date -Iseconds)" >"${CP1_PROGRESS_FILE}"
echo "[cp-init] starting control-plane bootstrap on $(hostname -s) at $(date -Iseconds)"
echo "[cp-init] settings: prepull=${CP1_PREPULL_IMAGES} prepull_timeout=${CP1_PREPULL_TIMEOUT_SECONDS}s init_timeout=${CP1_INIT_TIMEOUT_SECONDS}s progress_interval=${CP1_PROGRESS_INTERVAL_SECONDS}s"

actual_host="$(hostname -s 2>/dev/null || true)"
if [[ "${actual_host}" != "${EXPECTED_NODE_HOSTNAME}" ]]; then
  {
    echo "cp1 bootstrap identity precheck failed"
    echo "expected hostname: ${EXPECTED_NODE_HOSTNAME}"
    echo "actual hostname: ${actual_host:-<empty>}"
    echo "expected IP: ${CP1_IP}"
    echo "hint: vagrant machine mapping drift (cp1/apilb swap)"
...
run_kubeadm_init() {
  ...
  run_phase_with_progress "kubeadm-init" "${CP1_INIT_TIMEOUT_SECONDS}" \
  kubeadm init \
...
bootstrap_control_plane() {
  if [[ "${CP1_PREPULL_IMAGES}" == "true" ]]; then
    ...
    run_phase_with_progress "prepull-images" "${CP1_PREPULL_TIMEOUT_SECONDS}" \
