...
recreate_machine() {
  ...
  for attempt in $(seq 1 "${max_attempts}"); do
    rc=0
    "${VAGRANT_RETRY}" vagrant up "${machine}" --provider "${provider}" --no-provision 2>&1 | tee "${out_file}" || rc=${PIPESTATUS[0]}
    if [[ "${rc}" -eq 0 ]]; then
      break
    fi
...
up_machine_with_retry() {
  local target="$1"
  local out_file rc target_prefix attempt max_attempts
  out_file="$(mktemp)"
  target_prefix="${project_prefix}${target}"
  max_attempts=3
  rc=0

  for attempt in $(seq 1 "${max_attempts}"); do
    rc=0
    "${VAGRANT_RETRY}" vagrant up "${target}" --provider "${provider}" --no-provision 2>&1 | tee "${out_file}" || rc=${PIPESTATUS[0]}
    if [[ "${rc}" -eq 0 ]]; then
      break
    fi
    if grep -Eq "Name .+ of domain about to create is already taken|Volume for domain is already created" "${out_file}"; then
      echo "[ensure-node] stale libvirt resources detected for ${target}; cleanup and retry ${attempt}/${max_attempts}" >&2
      "${SCRIPT_DIR}/libvirt_cleanup.sh" "${target_prefix}" "${net_prefix}" || true
      sleep 2
      continue
    fi
    break
...
reconcile_with_api_lb() {
  ...
  up_machine_with_retry "api-lb" || true
  "${VERIFY}" "api-lb" "api-lb" "${net_prefix}.5" >/dev/null 2>&1 || true
  up_machine_with_retry "${machine}" || true
}
