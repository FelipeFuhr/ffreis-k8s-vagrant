...
ensure_vm() {
  ...
  for attempt in $(seq 1 "${max_attempts}"); do
    rc=0
    "${VAGRANT_RUN}" vagrant up "${machine}" --provider "${KUBE_PROVIDER}" --no-provision 2>&1 | tee "${out_file}" || rc=${PIPESTATUS[0]}
    if [[ "${rc}" -eq 0 ]]; then
      break
    fi
...
provision_named() {
  local machine="$1" names="$2" expected_host="$3" expected_ip="$4" expected_cpus="$5" expected_mem="$6" expected_role="$7"
  "${ROOT_DIR}/scripts/ensure_node_identity.sh" "${machine}" "${expected_host}" "${expected_ip}" "${KUBE_PROVIDER}" "${expected_cpus}" "${expected_mem}" "${expected_role}"
...
if [[ "${MODE}" == "full" || "${MODE}" == "infra" ]]; then
  if [[ "${KUBE_API_LB_ENABLED:-true}" == "true" ]]; then
    log_phase "machine-up api-lb"
    ensure_vm "api-lb" "api-lb" "${KUBE_API_LB_IP}" "${KUBE_API_LB_CPUS}" "${KUBE_API_LB_MEMORY}" "api-lb"
    log_phase "machine-provision api-lb"
    provision_named "api-lb" "api-lb" "api-lb" "${KUBE_API_LB_IP}" "${KUBE_API_LB_CPUS}" "${KUBE_API_LB_MEMORY}" "api-lb"
  fi

  log_phase "machine-up cp1"
  ensure_vm "cp1" "cp1" "${KUBE_NETWORK_PREFIX}.11" "${KUBE_CP_CPUS}" "${KUBE_CP_MEMORY}" "control-plane"
  log_phase "machine-provision cp1"
  provision_named "cp1" "base-common,cp-init" "cp1" "${KUBE_NETWORK_PREFIX}.11" "${KUBE_CP_CPUS}" "${KUBE_CP_MEMORY}" "control-plane"
  ...
  if [[ "${KUBE_CP_COUNT}" -gt 1 ]]; then
    for i in $(seq 2 "${KUBE_CP_COUNT}"); do
      cp="cp${i}"
      log_phase "machine-up ${cp}"
      ensure_vm "${cp}" "${cp}" "${KUBE_NETWORK_PREFIX}.$((10 + i))" "${KUBE_CP_CPUS}" "${KUBE_CP_MEMORY}" "control-plane"
      log_phase "machine-provision ${cp}"
      provision_named "${cp}" "base-common,cp-join" "${cp}" "${KUBE_NETWORK_PREFIX}.$((10 + i))" "${KUBE_CP_CPUS}" "${KUBE_CP_MEMORY}" "control-plane"
